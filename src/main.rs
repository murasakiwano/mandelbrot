...
/// origin. If `c` seems to be a member (if `c` didn't leave the circle in `limit`
...
fn pixel_to_point(
    ...
    Complex {
        ...
        im: upper_left.im - pixel.1 as f64 * height / bounds.1 as f64, // pixel.1 increases as we go down, but the imaginary component
